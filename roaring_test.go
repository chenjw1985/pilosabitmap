...
import (
	...
	"git.liveramp.net/select-product/go-common/roaring"
)
	...
	for n := 0; n < b.N; n++ {
		...
		for row := uint64(1); row < NumRows; row++ {
			for col := uint64(1); col < NumColums; col++ {
				bm.Add(row*ShardWidth + (col * MaxContainerVal))
			}
		}
	}
}
	...
	for n := 0; n < b.N; n++ {
		...
		for col := uint64(1); col < NumColums; col++ {
			for row := uint64(1); row < middle; row++ {
				bm.Add(row*ShardWidth + (col * MaxContainerVal))
				bm.Add((NumRows-row)*ShardWidth + (col * MaxContainerVal))
			}
		}
	}
}
...
func BenchmarkContainerInsideOut(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		for col := uint64(1); col < NumColums; col++ {
			for row := uint64(1); row <= middle; row++ {
				bm.Add((middle+row)*ShardWidth + (col * MaxContainerVal))
				bm.Add((middle-row)*ShardWidth + (col * MaxContainerVal))
			}
		}
	}
